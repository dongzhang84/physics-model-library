...
![Transformer vs 搬运工——架构对比](architecture_01_transformer_vs_carrier.png)
...
![可逆交换自动机——架构](architecture_02_swap_automaton.png)

> 一叠相邻对的门控交换,每层错开配对。交换保守这一对的球数、且是自己的逆,门 `g` 只读被冻结的格子 → **by construction 精确可逆 + 守恒**。但交换是**局部**的,整叠表达不了 BBS 的非局部进位。
...
![守恒搬运工——架构](architecture_03_conserving_carrier.png)

> 和 Test 1 一样的左→右扫描,但进位是整数球数 `k`,每个 cell 只能**吐球**(out=1, k→t−1)或**留球**(out=0, k→t)——保守 `格子 + 进位` 的仅有两种走法。所以守恒是结构性的;门只学"**何时**吐"(而这恰好还原出 BBS)。
