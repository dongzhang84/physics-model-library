...
**Test 4 —— 在一个真能把模型分开的 regime 里做公平对照。** 守恒搬运工 vs **RNN / LSTM / Mamba(SSM)**(都带扫描先验);Transformer 只当无结构参照。任务必须同时:(a) 给守恒搬运工留一个**非平凡**的残差,(b) **难到让自由吐位模型在 OOD 长度上到不了 100%**。加难旋钮:更大的进位容量 K(更大的有界状态)、更长的预测步数 T(直接学 T 步映射的 LSTM 比"学单步、套 T 次"的搬运工更吃亏)、更密/更大的孤立子、或更大的 OOD 跨度(测到 256/512)。
- **成功** = 找到一个 regime,使 RNN/LSTM/Mamba 的 OOD 准确率 < 100% → 守恒漂,而守恒搬运工在 ≥ 它们准确率的同时守恒平的 100%。**这**才是公平、且独家的主张。
- **失败** = 若没有合理 regime 能做到,就**诚实弱化主张**:结构性守恒只是个**安全网**,只在学习不完美时才有用——不是对强扫描模型的普遍胜利。
