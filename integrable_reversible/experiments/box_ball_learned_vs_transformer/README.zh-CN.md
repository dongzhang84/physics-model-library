...
五个 test,一个任务族(箱球系统 BBS)。问题是:可积式的结构能不能**被学出来**(而不是硬编),并买到一个普通序列模型买不到的东西?

- **稳的结果是结构性守恒。** 守恒搬运工靠 `k' = t − out` 把球数守恒做成 by construction 的精确保证;跨 5 个种子,它在每个长度都保持 **100 ± 0**,而自由吐位的扫描模型——组合型 GRU、LSTM、Mamba 族 SSM——随长度增长而漂(K=6 时最干净:守恒搬运工 100 ± 0,其余 → ~0)。这是**扫描模型之间**的一个**效率 + 保证**优势——**不是**打赢无结构的 Transformer(那是预期内、也不公平,只作参照保留)。
- **学出来的残差是真的,但幅度不大。** 在 finite-carrier BBS 上模型确实学会了用进位(无视进位的门追不上它),但多种子把这条准确率优势缩到 K=2 时的 ~4 分(94 ± 5 vs 89);只有在更难的 K=6 才又干净又大。
- **我们刻意不主张的:** 不是"打赢 Transformer";不是"没有 leak"(进位扫描 + 守恒记账是硬编的——见附录里那个先弃后修的 Test 3);不是"从零学出整条规则"。诚实的标题很窄,但它扛得住多种子。

**边界。** 单一可逆系统(BBS)、玩具规模、单批 5 种子。同一套"结构守恒 + 学残差"的配方能不能迁移到**别的**可逆系统(Margolus CA、Toda……),是很自然的下一个问题——但那是**另一个独立的实验**,不属于本实验。
