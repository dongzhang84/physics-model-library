...
2. **守恒是精确性的副产品——而且能单独*外挂*,所以它不是招牌。** 一个把规则学到精确的模型天然就守恒(LSTM 在 100% 准的地方就 100% 守恒,在准确率下滑处守恒立刻崩)。而且你可以不靠这套结构、直接把守恒强行加上:[Margolus 实验](../margolus_block_ca/)把守恒投影外挂到自由式模型上——它们的球数被钉死在 100%,可准确率在组合下*照样*崩。所以"结构守恒稳在 100"是*外挂约束就能复制的*;复制不了的是结构带来的那个**精确性**。
3. **Transformer 连"从左到右扫描"都没有,** 所以两个轴上都垫底。它是个不公平、无结构的参照——**不是**核心成果(任何扫描模型都能赢它,那只是复现了已知的"循环/状态追踪能外推、注意力不能")。

**诚实边界。** (a) 这是**扫描模型之间**的*归纳偏置*优势,不是"任何网络绝对做不到"。(b) 那个*精确性*只在 K=6 干净(100/100);在更容易的 K=2 学习模型是 94 ± 5,相对无视进位门的优势只有 ~4 分(Test 3)。(c) 进位扫描和守恒记账是硬编的——网络学的是残差,不是从零学整条规则(见附录里那个先弃后修的 Test 3)。(d) 外挂守恒那个对照是在 *Margolus* 系统上跑的、不是直接在 BBS 上;BBS 自己的 bolt-on 对照是最直接的确认(现已定为标准配置)。(e) 单一可逆系统、玩具规模、单批 5 种子;跨系统普适性(Margolus CA、Toda……)是**另一个独立**实验。
