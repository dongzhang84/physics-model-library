...
**多种子之后的结论:** 稳的主张是*结构性守恒保持精确(100 ± 0),而自由吐位扫描模型漂*——在 Test 4(K=6)最强最干净。*学习准确率*那点优势在 K=2(Test 3)偏小且抖、在 K=6(Test 4)干净。仍是单个任务族(BBS);跨系统普适性是 Test 6。
...
**Test 4 —— ✅ 已完成(见上面的 [Test 4](#test-4--和扫描模型的公平对照rnn--lstm--mamba) 一节)。** 单种子:在更难的 regime(K=6)下,**组合型自由吐 GRU 和直接映射模型(LSTM、Mamba 族 SSM)都匹配不上守恒搬运工**——GRU 连规则都学不满(89% 训练长度内)、守恒崩;LSTM 训练长度内学得会但 OOD 守恒漂;守恒搬运工全程 100/100。主张:*同等预算下,标准自由吐位扫描模型(组合型 + 直接映射)都匹配不上结构守恒的准确率+守恒——是效率 + 保证的优势*(不是"任何网络绝对做不到")。剩下的严谨性:多种子(Test 6)。

**Test 5 —— ✅ 已完成:多种子严谨性(见上面的 [Test 5](#test-5--多种子严谨性5-个种子正式数字) 一节)。** 四个 test 全部跑 5 个种子 → 均值 ± 标准差。它纠正了 Test 3 单种子的准确率(100 → 94±5)、把学习门 vs 无视进位门的差缩到 ~4 分,同时证实了稳的结果:结构性守恒保持 100 ± 0、而自由吐位模型漂(在 Test 4、K=6 最干净)。

**Test 6 —— 普适性(第二个可逆系统)。** 推荐 Margolus 邻域可逆 CA。同一套配方(结构守恒 + 学残差)换到**不同**系统——验证"配方可迁移"以及 proposal 那个预判(不同系统需要不同结构)。这是下一个实验。

**顺序:** Test 4(已完成)→ Test 5 多种子(已完成)→ Test 6 普适性(下一步)——先修地基,再盖二层。接上 [`../../proposal.md`](../../proposal.md) 里 `data/reversible_systems/` 的计划。
