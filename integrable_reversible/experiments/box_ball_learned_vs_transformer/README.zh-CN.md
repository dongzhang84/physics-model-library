...
**下一步 = 真正要紧的对照(公平对照,不是 Transformer)。** 把同一任务跟**同样带左→右扫描先验的模型**比——**RNN / LSTM / Mamba(SSM)**——比结构性守恒独家买到的那一样:**谁的守恒量精确、谁随长度漂。** 自由吐位的扫描会漂(Test 1 的普通搬运工已经显示:守恒往下掉);守恒搬运工是平的 100%。在准确率上赢无结构的 Transformer 是意料之中、且与已有结论重叠,所以它只当下界参照。然后做大:跨**多个可逆系统**(finite-carrier BBS、Margolus CA、Toda)+ 多种子 + 误差棒的**普适性研究**——即 [`../../proposal.md`](../../proposal.md) 里 `data/reversible_systems/` 的计划——才是把它从"干净的 demo"推向"可发表结果"的那一步。
