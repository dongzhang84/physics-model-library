...
**We ran 5 tests** (files `01`–`05`, one folder). Tests 1–3 build the model step by step (remove the cheat → weld the guarantees → the conserving carrier that actually learns); Test 4 pits it fairly against RNN / LSTM / Mamba; Test 5 re-runs all four across 5 seeds for formal mean ± std numbers. Together they are the proposal's "Route C" (a learned model with integrable structure). **Answer, stated with the multi-seed numbers: yes — the structure lets the rule be learned *exactly*, so it composes without drift (at K=6 the learned model holds 100 / 100 accuracy·conservation where free-emit scan models decay to 79–91%). Conservation itself turns out to be a *bolt-on-able* by-product — a follow-up control forces it onto free-form models and their accuracy still collapses — so the non-replicable win is *exactness under composition*, not the invariant. See the [Conclusion](#conclusion).**

> **"Beating a Transformer" is *not* the headline.** Any model here with a left→right scan beats the structure-free Transformer on length extrapolation — but that is *expected*, overlaps the known "recurrent / state-tracking models extrapolate, attention doesn't" result, and is *not a fair fight* (the Transformer has no scan prior). It is kept only as a no-structure reference. The **fair, discriminating comparison is against other scan models — RNN / LSTM / Mamba (SSM) — where the question is *whose prediction stays exact under long composition vs. drifts with length*.** Conservation alone is *not* that thing — it can be bolted onto any scan model (a follow-up control does exactly that, and their accuracy still drifts). What the structure uniquely buys is the **exactness** that composes without drift.
...
"Beating a Transformer" is expected and unfair (it has no scan prior), so the real test is against models that **also** scan left→right — a plain RNN, an LSTM, and a minimal diagonal SSM (a Mamba-family stand-in). The question is what the structure could uniquely buy: **whose prediction stays *exact* under long composition vs. drifts with length** (conservation alone is bolt-on-able — see the Conclusion). Because drift is a symptom of imperfect accuracy, the task is pushed to a harder regime — finite-carrier BBS with a **bigger bounded carrier (K=6)**, tested out to L=256 — so the free-emit models can't simply learn it perfectly.
...
**Bottom line after multi-seed:** the **robust** claim is *the structure's **exactness** composes without drift* — at K=6 the conserving carrier holds **100 / 100** while free-emit scan models decay to 79–91% accuracy (and their conservation to ~0). Conservation itself is a **bolt-on-able** by-product: the [Margolus experiment](../margolus_block_ca/) bolts a conservation projection onto free-form models and their accuracy *still* collapses — so the headline is *exactness under composition*, not the invariant. This is strongest/cleanest at Test 4 (K=6); the learned-accuracy edge is modest at K=2. Still one task family (BBS); cross-system generality is a separate experiment (see the Conclusion).
...
**Why it wins — the mechanism, stated plainly.** The win is **exactness under composition**, and it comes from inductive bias — *not* from the conservation guarantee per se:

1. **The structural counter lets the rule be learned *exactly*, so it composes without drift.** Because the carrier count is maintained by construction, the network only fits a small emit-gate — and that gate extrapolates (100% at L=256). GRU / LSTM / SSM must instead carry the counter *inside their hidden state*; a tiny per-step error accumulates over length, so their accuracy decays to 79–91%. This exactness is the thing free-form models cannot match.
2. **Conservation follows from exactness — and is separately *bolt-on-able*, so it is not the headline.** A model that predicts the rule exactly conserves for free (the LSTM conserves 100% exactly where it is 100% accurate, in-dist, and its conservation collapses where its accuracy decays, OOD). And you can force conservation directly without any of this structure — bolt a top-N projection onto the free-form models (`05_bolt_on.png`): their ball-count is pinned to **100%**, yet their accuracy is **unchanged** (GRU 79, LSTM 91, SSM 92 at L=256) and still short of the conserving carrier's **100%**. So "structural conservation stays at 100" is *replicable by a bolted-on constraint*; the non-replicable thing is the **exactness** the structure enables (here a 9–21-point accuracy gap; far larger — free-form collapses to ~50% — in the [Margolus experiment](../margolus_block_ca/), whose horizon grows with length so errors compound).
...
**Honest boundaries.** (a) This is an *inductive-bias* advantage **among scan models**, not "no network can ever do it." (b) The *exactness* is clean only at K=6 (100/100); at the easier K=2 the learned model is 94 ± 5, so the edge over a carrier-blind gate is a modest ~4 points (Test 3). (c) The carrier scan and conservation bookkeeping are hard-coded — the network learns the residual, not the whole rule from scratch (see the scrapped-then-fixed Test 3 in the appendix). (d) The bolt-on-conservation control is now run on BBS directly (`05_bolt_on.png`): forcing conservation onto the free-form scan models pins their ball-count to 100% but leaves accuracy essentially unchanged (79–92%), still below the conserving carrier's 100% — confirming the win is exactness, not the invariant. The gap is **modest here (9–21 pts)** because Test 4 composes only T=2; it is dramatic in Margolus (T ∝ L). (e) One reversible system, toy scale, a single 5-seed batch; cross-system generality (Margolus CA, Toda, …) is a **separate** experiment.

**Bolt-on control (`05_bolt_on.png`).** Forcing conservation onto the free-form scan models (top-N projection, same trained models) pins ball-count to 100% but leaves accuracy essentially unchanged — the *free* and *+conservation* accuracy lines overlap — and still below the conserving carrier's 100% (gap ~9 pts for LSTM/SSM, ~21 for GRU at L=256). **Conservation is bolt-on-able; the exactness that composes without drift is not.**

![bolting conservation onto BBS free-form models](05_bolt_on.png)
