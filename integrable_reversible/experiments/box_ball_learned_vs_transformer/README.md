...
**Honest boundaries.** (a) This is an *inductive-bias* advantage **among scan models**, not "no network can ever do it." (b) The *exactness* is clean only at K=6 (100/100); at the easier K=2 the learned model is 94 ± 5, so the edge over a carrier-blind gate is a modest ~4 points (Test 3). (c) The carrier scan and conservation bookkeeping are hard-coded — the network learns the residual, not the whole rule from scratch (see the scrapped-then-fixed Test 3 in the appendix). (d) The bolt-on-conservation control was run on the *Margolus* system, not on BBS directly; a BBS bolt-on control is the natural direct confirmation (now a standard control going forward). (e) One reversible system, toy scale, a single 5-seed batch; cross-system generality (Margolus CA, Toda, …) is a **separate** experiment.
