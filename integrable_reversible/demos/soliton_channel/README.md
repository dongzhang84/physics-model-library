...
![孤立子信道动画：上排 KdV 孤立子对撞后原样穿过，下排纯色散摊成碎波](soliton_channel.gif)

> **上排**：可积（KdV）信道——4 个孤立子对撞、穿过彼此，**高度形状原样**离开。**下排**：纯色散对照——同样 4 个波**摊开、糊掉**。动图自动循环；把整段演化压成一张的静态总览图见下方"图怎么看"。

---

## 为什么做这个实验（动机）

今天的 Transformer 靠 attention 让序列里不同位置的信息相互作用，但标准 attention 对"无损、可逆、守恒"这几件事**不给任何结构性保证**——它即便保住了信息，也是**近似地、从数据里学来的**，序列一长就漂：各份信息混着混着就糊了、丢了，也难以从结果精确倒推回输入。（不是说 Transformer 绝对做不到——你甚至能硬把它凑成可逆；关键在它**不保证**，而我们要的是**保证**。）我们想要的是另一种相互作用：让各份信息彼此作用，作用完之后**每一份仍毫发无损，并且整个过程能精确逆转**——而且这三样是**焊进结构里的保证，不靠训练、不随长度漂**。

由此我们想到了孤立子的传播。孤立子的碰撞恰好就是这样一个现成的物理原型：两个孤立子迎面撞上、叠到一起、再穿过彼此——位置变了，形状和高度分毫不差。上面的动图是个简单的 demo，我们设置了四个波，让四个波沿着同一个方向传播。

首先是按照 KdV 方式的传播，四个波有相互作用，但是作用完之后是**零损耗**。如果把波的传播换成去掉非线性项的普通色散方程，四个波撞完即散、信息尽失。这个对比说明：我们想要的"**相互作用却零损耗、可逆**"的操作在物理里真实存在，而且**专属于可积系统**。KdV 靠它的守恒结构让波碰撞而不失真，去掉非线性项，这一切就崩了。

当然，这还只是物理演示，里面没有学习、没有神经网络。它的意义是给出一个**存在性证明**。下一步，是把孤立子碰撞改造成一个**可训练的层**：可逆性由结构焊死，碰撞规则从数据里学，再拿它和 Transformer 正面对打，看这种不损耗的相互作用能否变成实打实的机器学习优势。
...
**动图（页首）**——真实时间轴，直接看到"追上 → 叠合（碰撞）→ 穿过 → 分开"。

**静态总览图（下）**——把整段演化压进一张图，**颜色由暗到亮 = 时间从早到晚**：

![静态总览：颜色 = 时间](soliton_channel_demo.png)

- **上图（可积 / KdV）**：高的孤立子追上矮的、在中间叠在一起（碰撞），然后又干净地分开，每个高度都还在 → 标题显示 `✓ all recovered`。
