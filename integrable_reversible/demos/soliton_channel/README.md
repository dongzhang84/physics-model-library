...
把"孤立子碰撞"工程化成神经网络之前，得先回答一个前置问题：**我们真正想要的那种机制，在物理上到底存不存在？**

今天的 Transformer 靠注意力让序列里不同位置的信息相互作用，但这种混合是**有损、不可逆**的——信息混着混着就糊了、丢了，也无法从结果精确倒推回输入。我们想要的是另一种相互作用：**让各份信息彼此作用，作用完之后每一份仍毫发无损，并且整个过程能精确逆转。** 孤立子的碰撞恰好就是这样一个现成的物理原型：两个孤立子迎面撞上、叠到一起、再穿过彼此——位置变了，形状和高度分毫不差。

这个实验就是把它亲手跑出来，钉死三件事：

1. **四个波确实碰了**（有相互作用，不是各走各的）；
2. **撞完四个数字仍被完整解回**（零损耗）；
3. **换成去掉非线性项的普通色散方程，四个波撞完即散、信息尽失**（这种"碰撞不损"专属于可积结构，不是随便什么波都有）。

之所以非要在可积系统上做，是因为"**相互作用却零损耗、且可逆**"这个组合是可积系统独有的馈赠——无穷多个精确守恒量、完全可逆、孤立子碰撞不损——而这正是我们想赋予神经网络、注意力却给不了的东西。

所以这个实验**不负责造那个网络**（那是后续可训练"孤立子层"的事），它只负责给整条路线一个**存在性证明**：我们梦寐以求的理想操作在物理中真实存在、可被验证、且唯可积系统能给——因此"从可积系统里开采这种操作"这条路，值得走。
